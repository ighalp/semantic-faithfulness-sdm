...
async def run_cached_analysis(state, cached_dist):
    ...
    try:
        ...
        state['log_area'].push(f'  F_S (Semantic Faithfulness): {result["F_S"]:.4f}')
        state['log_area'].push(f'  SEP (Semantic Entropy Production): {result["D_min"]:.4f} bits')
        state['log_area'].push(f'  Ṡ (System Entropy Change): {entropy(p_a, base=2) - entropy(p_c, base=2):.4f} bits')
...
async def run_analysis(state):
    ...
    try:
        ...
        state['log_area'].push(f'SEP (Semantic Entropy Production): {results.SEP_total:.4f} bits')
        state['log_area'].push(f'Ṡ (System Entropy Change): {results.SEP_system:.4f} bits')
